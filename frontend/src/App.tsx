...
function RulesManager({ catalog, rules, reload, report, announce }: { catalog: LabelDimension[]; rules: MerchantRule[]; reload: () => Promise<void>; report: (reason: unknown) => void; announce: (value: string) => void }) {
  const emptyDraft = { name: '', source_platform: '', counterparty_exact: '', label_ids: [] as number[], notes: '', amount_min: '', amount_max: '', amount_scope: 'inside' as 'inside' | 'outside' }
  ...
  const amountInvalid = draft.amount_min !== '' && draft.amount_max !== '' && Number(draft.amount_min) > Number(draft.amount_max)
  ...
  const amountSummary = (rule: MerchantRule) => {
    if (rule.amountMin == null && rule.amountMax == null) return ''
    const range = `${rule.amountMin ?? 0}–${rule.amountMax ?? '∞'} 元`
    return rule.amountScope === 'outside' ? `金额不在 ${range}内` : `金额限 ${range}`
  }
  ...
  return <section className="workspace-page"><div className="workspace-heading"><p className="eyebrow">EXACT MATCH RULES</p><h1>让重复出现的商户自动填好建议</h1><p>只匹配交易对方完整名称；不扫描关键词，不读取商品说明。新规则先待应用，点击一次“应用到已有流水”后才生效。人工确认永远优先。</p></div><div className="rules-grid"><form className="rule-form" onSubmit={(event) => { event.preventDefault(); void run(() => createRule({ ...draft, amount_min: draft.amount_min === '' ? null : draft.amount_min, amount_max: draft.amount_max === '' ? null : draft.amount_max }), '预填规则已创建，尚未应用到已有流水'); setDraft(emptyDraft) }}><h2>新建精确规则</h2><label>规则名称<input required value={draft.name} onChange={(e) => setDraft({ ...draft, name: e.target.value })} /></label><label>平台<select value={draft.source_platform} onChange={(e) => setDraft({ ...draft, source_platform: e.target.value })}><option value="">不限平台</option><option>微信</option><option>支付宝</option></select></label><label>交易对方完整名称<input required value={draft.counterparty_exact} onChange={(e) => setDraft({ ...draft, counterparty_exact: e.target.value })} placeholder="必须与账单原值完全一致" /></label><fieldset className="rule-amount"><legend>金额范围（可选）</legend><div className="rule-amount-row"><select aria-label="金额范围方式" value={draft.amount_scope} onChange={(e) => setDraft({ ...draft, amount_scope: e.target.value as 'inside' | 'outside' })}><option value="inside">包括范围内</option><option value="outside">不包括范围内</option></select><label className="rule-amount-bound">下限<input aria-label="金额下限" type="number" min="0.01" step="0.01" inputMode="decimal" placeholder="不限" value={draft.amount_min} onChange={(e) => setDraft({ ...draft, amount_min: e.target.value })} /></label><span>至</span><label className="rule-amount-bound">上限<input aria-label="金额上限" type="number" min="0.01" step="0.01" inputMode="decimal" placeholder="不限" value={draft.amount_max} onChange={(e) => setDraft({ ...draft, amount_max: e.target.value })} /></label></div>{amountInvalid && <small>下限不能大于上限</small>}</fieldset><label>备注<textarea value={draft.notes} onChange={(e) => setDraft({ ...draft, notes: e.target.value })} /></label><button type="button" className="rule-label-trigger" onClick={() => setPickerOpen(true)}><span className="rule-trigger-head"><span>预填标签</span><b>{draft.label_ids.length ? `已选 ${draft.label_ids.length} 个` : '未选择'}</b></span><small>{selectedNames || '打开整面标签面板选择；可留空，命中时只确认商户'}</small></button><p className="rule-form-note">创建后规则保持“待应用”，不会影响已有流水；在右侧对它点击一次“应用到已有流水”，才会对所有匹配数据生效一次并转入生效中。</p><button className="button primary" disabled={amountInvalid}>创建规则</button></form><section className="rule-list"><h2>现有规则 <small>{rules.length}</small></h2>{rules.length ? rules.map((rule) => { const state = stateOf(rule); const amount = amountSummary(rule); return <article key={rule.id}><div><span className={state.className} title={state.hint}>{state.text}</span><h3>{rule.name}</h3><p>{rule.sourcePlatform || '全平台'} · “{rule.counterpartyExact}”</p>{amount && <small>{amount}</small>}<small>{rule.labelIds.map((id) => labelNameById.get(id)).filter(Boolean).join('、') || '未配置标签'}</small></div><div>{!rule.appliedAt && <button className="text-button" disabled={applyingId === rule.id} onClick={() => void applyOnce(rule)}>{applyingId === rule.id ? '应用中…' : '应用到已有流水'}</button>}<button className="text-button" onClick={() => void run(() => updateRule(rule.id, { enabled: !rule.enabled }), '规则状态已更新')}>{rule.enabled ? '停用' : '启用'}</button><button className="danger-link" onClick={() => { if (window.confirm('删除这条规则？')) void run(() => deleteRule(rule.id), '规则已删除') }}>删除</button></div></article> }) : <EmptyState text="还没有商户精确规则" />}</section></div>{pickerOpen && <RuleLabelPicker catalog={catalog} selected={draft.label_ids} onToggle={toggle} onClear={() => setDraft((value) => ({ ...value, label_ids: [] }))} onClose={() => setPickerOpen(false)} />}</section>
}
